...
export async function GET(request: NextRequest) {
  try {
    ...
    const result = await fetchFromBackend(
      'https://localhost:5001/api/v1/users?PageNumber=1&PageSize=50',
      token
    );

    if (!result.ok) {
      return NextResponse.json(errorResponse(result.errorText || 'Failed to fetch users'), { status: result.status });
    }
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!result.ok) {
      ...
      return NextResponse.json(errorResponse((errorData?.message as string) || 'Failed to create user'), { status: result.status });
    }
